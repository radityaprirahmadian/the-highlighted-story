...
const Introduction = () => {
  const [activeData, setActiveData] = useState({ id: 0, text: "" });

  return (
    <AnimatePresence>
      <motion.div className="w-full">
        <Hero active={activeData}></Hero>
        <motion.div
          initial={{ x: -2000 }}
          animate={{ x: 0 }}
          transition={{ ease: "easeInOut", duration: 1, stiffness: 100, delay: 0.2, type: "spring", damping: 12 }}
          className="flex flex-wrap items-center justify-between mt-16 gap-4"
        >
          {data.map((item, index) => {
            return (
              <motion.div
                key={index}
                className="basis-5/12 md:basis-2/12 lg:basis-1/12 relative aspect-square"
